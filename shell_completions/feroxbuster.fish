...
complete -c feroxbuster -l post-data -d 'Set -H \'Content-Type: application/x-www-form-urlencoded\' and --data to <post-data>' -r
complete -c feroxbuster -l post-form -d 'Set -H \'Content-Type: multipart/form-data; boundary=----WebKitFormBoundaryXXX\' and --data to <post-form>' -r
complete -c feroxbuster -l post-json -d 'Set -H \'Content-Type: application/json\', and --data to <post-json>' -r
