...
Register-ArgumentCompleter -Native -CommandName 'feroxbuster' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'feroxbuster' {
            ...
            [CompletionResult]::new('-F', 'F', [CompletionResultType]::ParameterName, 'Output file''s format (csv, json, text)')
            [CompletionResult]::new('--format', 'format', [CompletionResultType]::ParameterName, 'Output file''s format (csv, json, text)')
            ...
            [CompletionResult]::new('--json', 'json', [CompletionResultType]::ParameterName, 'Emit JSON logs to --output and --debug-log instead of normal text. Consider using (--format json) instead')
