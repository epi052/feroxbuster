...
Register-ArgumentCompleter -Native -CommandName 'feroxbuster' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'feroxbuster' {
            ...
            [CompletionResult]::new('-w', '-w', [CompletionResultType]::ParameterName, 'Path(s) or URL(s) of the wordlist(s); may be supplied multiple times (ex: -w a.txt -w b.txt) or as a comma-separated list (ex: -w a.txt,b.txt). Words from every source are merged and de-duplicated.')
            [CompletionResult]::new('--wordlist', '--wordlist', [CompletionResultType]::ParameterName, 'Path(s) or URL(s) of the wordlist(s); may be supplied multiple times (ex: -w a.txt -w b.txt) or as a comma-separated list (ex: -w a.txt,b.txt). Words from every source are merged and de-duplicated.')
            ...
            [CompletionResult]::new('-U', '-U ', [CompletionResultType]::ParameterName, 'Update feroxbuster to the latest version (--insecure and --server-certs are honored)')
            [CompletionResult]::new('--update', '--update', [CompletionResultType]::ParameterName, 'Update feroxbuster to the latest version (--insecure and --server-certs are honored)')
