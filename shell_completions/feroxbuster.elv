...
set edit:completion:arg-completer[feroxbuster] = {|@words|
    ...
    var completions = [
        &'feroxbuster'= {
            ...
            cand --post-data 'Set -H ''Content-Type: application/x-www-form-urlencoded'' and --data to <post-data>'
            cand --post-form 'Set -H ''Content-Type: multipart/form-data; boundary=----WebKitFormBoundaryXXX'' and --data to <post-form>'
            cand --post-json 'Set -H ''Content-Type: application/json'', and --data to <post-json>'
